const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5760';
...
const adminCookie = await login('admin@personal-care-manufacturing.local', 'admin123');
const managerCookie = await login('manager@personal-care-manufacturing.local', 'manager123');
const analystCookie = await login('analyst@personal-care-manufacturing.local', 'analyst123');
